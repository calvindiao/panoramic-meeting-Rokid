...
namespace Rokid.UXR.Demo
{
    public class VirtualControllerSample : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (RKInput.Instance.GetKeyUp(RKKeyEvent.KEY_OK))
            {
                cube.transform.localPosition = new Vector3(0, 1, 0);
            }

            if (RKInput.Instance.GetKey(RKKeyEvent.KEY_X))
            {
                cube.transform.Rotate(new Vector3(0, rotateSpeed, 0));
            }

            if (RKInput.Instance.GetKey(RKKeyEvent.KEY_B))
            {
                cube.transform.Rotate(new Vector3(0, -rotateSpeed, 0));
            }

            if (RKInput.Instance.GetKeyDown(RKKeyEvent.KEY_Y))
            {
                colorIndex++;
                cube.GetComponent<MeshRenderer>().material.color = colors[Mathf.Abs(colorIndex) % colors.Count];
            }

            if (RKInput.Instance.GetKeyDown(RKKeyEvent.KEY_A))
            {
                colorIndex--;
                cube.GetComponent<MeshRenderer>().material.color = colors[Mathf.Abs(colorIndex) % colors.Count];
            }

            if (RKInput.Instance.GetKey(RKKeyEvent.KEY_LEFT) || Input.GetKeyDown(KeyCode.LeftArrow))
            {
                cube.transform.Translate(Vector3.left * moveSpeed * Time.deltaTime, Space.World);
            }

            if (RKInput.Instance.GetKey(RKKeyEvent.KEY_RIGHT) || Input.GetKeyDown(KeyCode.RightArrow))
            {
                cube.transform.Translate(Vector3.right * moveSpeed * Time.deltaTime, Space.World);
            }

            if (RKInput.Instance.GetKey(RKKeyEvent.KEY_UP) || Input.GetKeyDown(KeyCode.UpArrow))
            {
                cube.transform.Translate(Vector3.up * moveSpeed * Time.deltaTime, Space.World);
            }

            if (RKInput.Instance.GetKey(RKKeyEvent.KEY_DOWN) || Input.GetKeyDown(KeyCode.DownArrow))
            {
                cube.transform.Translate(Vector3.down * moveSpeed * Time.deltaTime, Space.World);
            }


            //Debug.Log("UXR-PLUGIN::lr axis = " + RKInput.GetAxis(AxisEvent.Horizontal_Left) + ", " + RKInput.GetAxis(AxisEvent.Vertical_Left));
            leftRockerInfo.text = "Left Horizontal Axis: " + RKInput.Instance.GetAxis(AxisEvent.Horizontal_Left) + ", Vertical Axis: " + RKInput.Instance.GetAxis(AxisEvent.Vertical_Left);
            rightRockerInfo.text = "Right Horizontal Axis: " + RKInput.Instance.GetAxis(AxisEvent.Horizontal_Right) + ", Vertical Axis: " + RKInput.Instance.GetAxis(AxisEvent.Vertical_Right);
        }
    }
}